...
function bread_crumbs($class = '', $lis = [], $li_replase = [], $title = null) {
    ...
    $li =[
        [get_page_link(8), __('Головна','reverside')],
        ['', get_the_title()]
    ];
...
//    $_POST['house'], $_POST['floor']
    ...
    $flatPage .= " <div class='flat-parameters'><h1>ID-".$id."</h1>
                <div class='flat-parameters__general-block'>
                    <div class='flat-parameters__title common-subtitle'>".__('Параметри:','reverside')."</div>
                    <div class='total-square'>".__('Загальна:','reverside')."<span class='square-digit'>
                 ".$data['data']['data'][0]['all_room']."м<sub>2</sub></span></div>
                    <div class='living-square'>".__('Житлова:','reverside')."<span class='square-digit'>".$data['data']['data'][0]['life_room']." м<sub>2</sub></span></div>
                </div>
                <div class='flat-parameters__rooms-block'>";
    foreach ($data['data']['data'][0]['properties'] as $room){
        $flatPage .="<div class='room-type'>
                        <span>".__($room['property_name'],'reverside')."</span>
  ...
  <a class='button flat-price-info-button form-js'>
   <span class='button_text'>".__('Дізнатись ціну','reverside')."</span>
  </a>
  <a class='transparent-button flat-transparent-pdf' target='_blank' href='/pdf?flat=".$data['data']['data'][0]['id']."&floor=".$data['data']['data'][0]['floor']."'>".__('завантажити PDF','reverside')."</a>
</div>
<div class='flat-group2'>
    ...
    $flatPage .="
            <a class='flat-button-return' href='/choose-floor/'>
              ".__('Вибрати іншу квартиру','reverside')."
            </a>
